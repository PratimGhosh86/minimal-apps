...
public class Najardari {
	...
	static void kichuEktaKorteiHobe() throws AWTException, InterruptedException {
		if (ektaSonkhyaDaoToErBhetore(0, 9) % 3 == 0)
			janlaPaltao();
		else
			idurBhagao();
		idurTaKothayeAcheDekhaJak();
	...
	static void idurBhagao() throws AWTException {
		if (idurKiNoraNoriKoreni()) {
			...
			tukKoreHurkiyeJao( //
					mLoc.x, ektaSonkhyaDaoToErBhetore(noraChorarSimana.get("chotoX"), noraChorarSimana.get("boroX")), //
					mLoc.y, ektaSonkhyaDaoToErBhetore(noraChorarSimana.get("chotoY"), noraChorarSimana.get("boroY")), //
					1, 1000 //
			);
		}
	}

	static void tukKoreHurkiyeJao(final int shururX, final int sesherX, final int shururY, final int sesherY,
			final int somoy, final int dhaap) throws AWTException {
		final var robo = new Robot();
		final var dx = (sesherX - shururX) / ((double) dhaap);
		final var dy = (sesherY - shururY) / ((double) dhaap);
		final var dt = somoy / ((double) dhaap);
	...
	static void dekhoToKonJanlaTaKhola(final Robot robo) {
		...
		if (naam.contains("Eclipse") || naam.contains("Spring Tool Suite"))
			...
			normalJanlaTeNaraNariKoro(robo);
